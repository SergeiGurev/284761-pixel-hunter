...
const leftCheckboxes = game1Element.querySelectorAll(`[name="question1"]`);
const rightCheckboxes = game1Element.querySelectorAll(`[name="question2"]`);

leftCheckboxes.forEach((checkbox) => {
  checkbox.addEventListener(`click`, () => {
    if (rightCheckboxes[0].checked || rightCheckboxes[1].checked) {
...
rightCheckboxes.forEach((checkbox) => {
  checkbox.addEventListener(`click`, () => {
    if (leftCheckboxes[0].checked || leftCheckboxes[1].checked) {
      showScreen(game2Element);
    }
  });
});

backToGreeting(game1Element);
